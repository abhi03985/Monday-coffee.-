...
city_name,
round(
(population * 0.25)/1000000,2) as coffee_consumer_in_millions,
city_rank
from city
order by 2 desc
...
WITH city_metrics AS (
    SELECT 
        ...
        COUNT(DISTINCT s.customer_id) AS total_cx,
        ROUND(
            CAST(SUM(s.total) AS DECIMAL(10,2)) / CAST(COUNT(DISTINCT s.customer_id) AS DECIMAL(10,2)), 
            2
        ) AS avg_sale_pr_cx
    FROM sales AS s
...
SELECT 
    ...
    avg_sale_pr_cx,
    ROUND(
        CAST(estimated_rent AS DECIMAL(10,2)) / CAST(total_cx AS DECIMAL(10,2)), 
        2
    ) AS avg_rent_per_cx
FROM city_metrics
...
SELECT 
    ...
    avg_sale_pr_cx,
    ROUND(estimated_rent / total_cx, 2) AS avg_rent_per_cx
FROM city_metrics
ORDER BY total_revenue DESC;
